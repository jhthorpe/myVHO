...
REAL(KIND=8) FUNCTION term_2_aux(i,j,a,bra,ket,phi2,zeta)
  IMPLICIT NONE
  REAL(KIND=8), DIMENSION(0:,0:,0:), INTENT(IN) :: zeta
 ...
 ! WRITE(*,*) "ji ij", val
  ...
  val = val + zeta(j,i,a)*zeta(j,i,a)*phi2(i)/phi2(j)*&
              ...
              ints_q(bra(j),ket(j))*ints_p(bra(i),ket(i))
 ! WRITE(*,*) "ji ji", val
  term_2_aux = -1.0D0*val !- accounts for the two p integrals
END FUNCTION term_2_aux
...
REAL(KIND=8) FUNCTION term_2_old(nvib,a,ket,Be,phi2,zeta)
  IMPLICIT NONE
  REAL(KIND=8), DIMENSION(0:,0:,0:), INTENT(IN) :: zeta
  REAL(KIND=8), DIMENSION(0:), INTENT(IN) :: Be,phi2
  INTEGER, DIMENSION(0:), INTENT(IN) :: ket
...
!      WRITE(*,*) "<",bra(0:nvib-1),"|",Ek,p1,p2
  ...
  term_2_old = temp*4.0D0*Be(a)*Be(a)
  
END FUNCTION term_2_old
...
REAL(KIND=8) FUNCTION term_3(nvib,a,ket,phi2,phi3,mu1)
  IMPLICIT NONE
