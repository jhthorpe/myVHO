...
! b             : int, rotational axis β 
! ket           : 1D int, ket QN's
! mu2           : 4D real*8, second order mu matrix

REAL(KIND=8) FUNCTION term_1(nvib,a,b,ket,mu2)
  IMPLICIT NONE
  ...
  bra = ket
  DO j=0,nvib-1
    temp = temp + mu2(j,j,a,b)*ints_qq(bra(j),ket(j))
  END DO
  term_1 = 0.25D0*temp
...
REAL(KIND=8) FUNCTION term_3(nvib,a,b,ket,phi2,phi3,mu1)
  IMPLICIT NONE
  ...
  DO i=0,nvib-1
    ...
    IF (bra(i) .GE. 0) THEN
      Ek = Ei - phi2(i)
      temp = temp + mu1(i,a,b)*ints_q(ket(i),bra(i))*&
